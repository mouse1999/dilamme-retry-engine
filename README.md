...
A high-throughput, non-blocking HTTP request processing engine built with **Spring Boot 3.x** and **SQLite**. It handles unreliable downstream APIs by processing requests asynchronously, applying exponential backoff with full jitter, and tracking full lifecycle execution state across a persistent attempt history.
